...
<div class="row p-2">
    ...
    $Sci_con=$conn->prepare("SELECT * FROM our_service order by id DESC LIMIT 50");
    $Sci_con->execute();
    $cnt = $Sci_con->rowCount();
    if ($cnt < 1) {

echo '<center><h2 class="p-3 m-3 fw-bold text-danger"> No Services Found </h2></center>';
...
<div class="d-flex p-3">
  ...
  <div class="d-flex align-items-center">
    ...
    <button class="btn btn-primary bg-danger w-100" style="border:none; margin-left: 20px;font-weight: bolder;" onclick="Sci.delete('<?php echo $Sci_id; ?>','deleteour_service.php')">Delete</button></div>
    <button class="btn btn-primary bg-twist-bold w-100" style="border:none; margin-left: 20px;font-weight: bolder;" onclick=" $space_load('our_service_edit.php?topic=<?php echo $Sci_id; ?>','sign')">Edit</button></div>
  </div>
     </div>
